...
export function getDiamondSystem({
	...
	const intersectSphere: any = Fn(([origin, direction]: any) => {
		...
		If(greaterThan(disc, 0.0), () => {
			...
			const t1 = div(
				mul(add(negate(B), sqrtDisc), uniforms.geometryFactor),
				A,
			);
			const t2 = div(
	...
	const traceRay: any = Fn(([origin, direction, normal]: any) => {
		...
		Loop({ start: int(0), end: int(end) }, ({ i }) => {
			...
			attenuationFactor.assign(
				...
				mul(
					attenuationFactor,
					exp(negate(mul(r, uniforms.absorbption))),
				),
			);
			...
			If(equal(dot(refractResult, refractResult), 0.0), () => {
				...
				If(equal(i, int(end - 2)), () => {
					...
					const contrib = mul(
						...
						mul(
							mul(
								mul(uniforms.correction, attenuationFactor),
								uniforms.boost,
							),
				...
				outColor.addAssign(
					mul(
						mul(
							mul(combinedColor, uniforms.correction),
							attenuationFactor,
						),
						uniforms.boost,
					),
				...
				attenuationFactor.assign(
					mul(
						mul(attenuationFactor, brdfReflectedIn),
						uniforms.boost,
					),
				);
